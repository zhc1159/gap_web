    ...
    <div class="main-card">
      <div class="card-content">
        <div v-if="selectedRows.length > 0" class="batch-bar">
          <span class="batch-info">{{ $t('opc.cip.selectedCount', { count: selectedRows.length }) }}</span>
          <el-button type="danger" size="small" @click="handleBatchDelete">
            <el-icon><Delete /></el-icon>
            {{ $t('common.batchDelete') }}
          </el-button>
        </div>
        <el-table :data="tableData" v-loading="loading" class="cip-table" @selection-change="handleSelectionChange">
...
// ==================== 状态 ====================
const selectedRows = ref<CipRule[]>([])
const handleSelectionChange = (rows: CipRule[]) => {
...
const handleBatchDelete = async () => {
  try {
    await ElMessageBox.confirm(
      t('opc.cip.batchDeleteConfirm', { count: selectedRows.value.length }),
      t('common.confirm'),
      { type: 'warning' }
    )
    ...
    selectedRows.value = []
    fetchList()
    ElNotification({ title: t('common.success'), message: t('opc.cip.batchDeleteSuccess'), type: 'success', customClass: 'notification-success' })
  } catch { /* cancel */ }
}
