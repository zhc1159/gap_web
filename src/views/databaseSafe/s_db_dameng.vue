      ...
      <div class="card main-card">
        <div class="card-content">
          ...
          <div v-if="selectedRows.length > 0" class="batch-bar">
            <span class="batch-info">{{ $t('databaseSafe.s_db_dameng.selectedCount', { count: selectedRows.length }) }}</span>
          ...
          <!-- 表格区域 -->
          <el-table :data="tableData" v-loading="loading" class="dameng-table" @selection-change="handleSelectionChange">
...
const selectedRows = ref<DamengSecurityConfig[]>([])

const handleSelectionChange = (rows: DamengSecurityConfig[]) => {
...
const handleBatchDelete = async () => {
  try {
    await ElMessageBox.confirm(
      t('databaseSafe.s_db_dameng.batchDeleteConfirm', { count: selectedRows.value.length }),
      t('common.confirm'),
      { type: 'warning' }
    )
    ...
    selectedRows.value = []
    fetchList()
    ElNotification({ title: t('common.success'), message: t('databaseSafe.s_db_dameng.batchDeleteSuccess'), type: 'success', customClass: 'notification-success' })
  } catch { /* cancel */ }
}
