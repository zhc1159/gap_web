<template>
  <div class="page-container">
    <div class="page-card">
      <div class="page-describe">{{ $t('audit.ifcfgStatusLog.describe') }}</div>
      <div class="page-util">
        <el-form :model="filterForm" inline class="form-label">
          <el-form-item :label="$t('audit.ifcfgStatusLog.interface')">
            <el-input v-model="filterForm.interface" :placeholder="$t('audit.ifcfgStatusLog.interfacePlaceholder')" clearable />
          </el-form-item>
          ...
          <el-form-item>
            <el-dropdown @command="handleAction">
              ...
              <template #dropdown>
                <el-dropdown-menu>
                  <el-dropdown-item command="clear">{{ $t('audit.ifcfgStatusLog.clearLog') }}</el-dropdown-item>
                </el-dropdown-menu>
              </template>
            </el-dropdown>
          </el-form-item>
      ...
      <div class="page-table">
        <el-table :data="tableData" v-loading="loading" stripe>
          ...
          <el-table-column prop="date" :label="$t('audit.ifcfgStatusLog.time')" width="180" sortable />
          <el-table-column prop="interface" :label="$t('audit.ifcfgStatusLog.interface')" width="120">
            <template #default="{ row }">
              <el-tag type="primary" size="small">{{ row.interface }}</el-tag>
            </template>
          </el-table-column>
          <el-table-column prop="oldStatus" :label="$t('audit.ifcfgStatusLog.oldStatus')" width="100">
            <template #default="{ row }">
              <el-tag :type="row.oldStatus === 'up' ? 'success' : 'danger'" size="small">
                {{ row.oldStatus === 'up' ? $t('audit.ifcfgStatusLog.up') : $t('audit.ifcfgStatusLog.down') }}
              </el-tag>
            </template>
          </el-table-column>
          <el-table-column prop="newStatus" :label="$t('audit.ifcfgStatusLog.newStatus')" width="100">
            <template #default="{ row }">
              <el-tag :type="row.newStatus === 'up' ? 'success' : 'danger'" size="small">
                {{ row.newStatus === 'up' ? $t('audit.ifcfgStatusLog.up') : $t('audit.ifcfgStatusLog.down') }}
              </el-tag>
            </template>
          </el-table-column>
          <el-table-column prop="ip" label="IP Address" width="150" />
          <el-table-column prop="mac" label="MAC Address" width="150" />
          <el-table-column prop="describe" :label="$t('audit.ifcfgStatusLog.description')" min-width="200" show-overflow-tooltip />
        </el-table>
...
interface IfcfgStatusItem { id: string; date: string; interface: string; oldStatus: string; newStatus: string; ip: string; mac: string; describe: string }

const filterForm = reactive({ interface: '', status: '', dateRange: [] as string[] })
...
const fetchData = async () => {
  ...
  try {
    ...
    tableData.value = [
      { id: '1', date: '2026-04-07 10:30:15', interface: 'eth0', oldStatus: 'down', newStatus: 'up', ip: '192.168.1.100', mac: '00:1A:2B:3C:4D:5E', describe: 'Interface eth0 brought up' },
      { id: '2', date: '2026-04-07 10:25:42', interface: 'eth1', oldStatus: 'up', newStatus: 'down', ip: '192.168.2.100', mac: '00:1A:2B:3C:4D:5F', describe: 'Interface eth1 link down detected' },
      { id: '3', date: '2026-04-07 10:20:33', interface: 'eth2', oldStatus: 'down', newStatus: 'up', ip: '10.0.0.100', mac: '00:1A:2B:3C:4D:60', describe: 'Interface eth2 connected' },
      { id: '4', date: '2026-04-07 10:15:18', interface: 'eth0', oldStatus: 'up', newStatus: 'up', ip: '192.168.1.100', mac: '00:1A:2B:3C:4D:5E', describe: 'IP address updated to 192.168.1.100' },
      { id: '5', date: '2026-04-07 10:10:05', interface: 'lo', oldStatus: 'up', newStatus: 'up', ip: '127.0.0.1', mac: '00:00:00:00:00:00', describe: 'Loopback interface verified' }
    ]
    pagination.total = 50
...
const handleAction = async (command: string) => {
  if (command === 'clear') {
    try {
      await ElMessageBox.confirm(t('audit.ifcfgStatusLog.clearLogConfirm'), t('common.warning'), { confirmButtonText: t('common.confirm'), cancelButtonText: t('common.cancel'), type: 'warning' })
      tableData.value = []; pagination.total = 0
      ElNotification({ title: t('common.success'), message: t('audit.ifcfgStatusLog.clearSuccess'), type: 'success', customClass: 'notification-success' })
    } catch { }
  }
}
const handleExport = async (type: string) => {
  await new Promise(resolve => setTimeout(resolve, 300))
  ElNotification({ title: t('common.success'), message: t('audit.ifcfgStatusLog.exportSuccess') + ` (${type})`, type: 'success', customClass: 'notification-success' })
}
