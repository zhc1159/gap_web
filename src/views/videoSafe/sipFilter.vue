      ...
      <div class="card main-card">
        <div class="card-content">
          ...
          <div v-if="selectedRows.length > 0" class="batch-bar">
            <span class="batch-info">{{ $t('videoSafe.sipFilter.selectedCount', { count: selectedRows.length }) }}</span>
          ...
          <!-- 表格区域 -->
          <el-table :data="tableData" v-loading="loading" class="sip-table" @selection-change="handleSelectionChange">
...
const selectedRows = ref<SIPFilterRule[]>([])
...
const handleSelectionChange = (rows: SIPFilterRule[]) => {
...
const handleBatchDelete = async () => {
  try {
    await ElMessageBox.confirm(
      t('videoSafe.sipFilter.batchDeleteConfirm', { count: selectedRows.value.length }),
      t('common.confirm'),
      { type: 'warning' }
    )
    ...
    selectedRows.value = []
    fetchList()
    ElNotification({ title: t('common.success'), message: t('videoSafe.sipFilter.batchDeleteSuccess'), type: 'success', customClass: 'notification-success' })
  } catch { /* cancel */ }
}
